...
import { ListRescheduleAdmin } from "@/app/dashboard/admin/booking/reschedule/[id]/client";

export default async function HalamanRescheduleAdmin(props: {
  ...
  // Ambil data pemesanan (tanpa filter penggunaId agar admin bisa reschedule siapa saja)
  const pesanan = await prisma.pemesanan.findUnique({
    where: { idPemesanan: params.id },
  ...
  return (
    <>
      <div className="konten-header" style={{ justifyContent: "space-between" }}>
        <div>
          <h1 style={{ marginBottom: "0.25rem" }}>Reschedule Booking (Admin)</h1>
          <p style={{ fontSize: "0.9rem", color: "var(--abu-500)" }}>
            Mengubah jadwal untuk pesanan <strong>#{pesanan.kodePemesanan}</strong>
          </p>
        </div>
      ...
      <div className="konten-body">
        ...
        <ListRescheduleAdmin jadwalLapangan={jadwal} pemesananId={pesanan.idPemesanan} />
      </div>
    </>
  );
}
